...
def consistent(assignment, csp, var, value):
	"""Question 1
		This function indicates whether a given value would be possible to assign to a variable without violating any of
		its constraints. You only need to consider the constraints in csp.binaryConstraints that affect this variable
		and have the other affected variable already assigned
	"""
	"""Your Code Here"""
	...
	for constraint in csp.binaryConstraints:
		if constraint.var1 == var:
			if not constraint.isSatisfied(value, assignment.assignedValues[constraint.var2]):
				return False

		if constraint.var2 == var:
			if not constraint.isSatisfied(assignment.assignedValues[constraint.var1], value):
				return False
	return True
...
def recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod):
	"""Question 1
		his function is designed to take in a problem definition and a partial assignment. When finished, the assignment
		should either be a complete solution to the CSP or indicate failure"""
	"""YOUR CODE HERE"""
	#psuedocode from chapter 6.3 on backtracking, but without inference
	if assignment.isComplete():
		return assignment

	var = selectVariableMethod(assignment, csp)
	for value in orderValuesMethod(assignment, csp, var):
		#check value of variable to ensure consistency with binary constraint
...
def minimumRemainingValuesHeuristic(assignment, csp):
	"""Question 2
		This follows the minimum remaining value heuristic to select a variable with the fewest options left and uses the
		degree heuristic to break ties"""
	"""YOUR CODE HERE"""
	nextVar = None
	domains = assignment.varDomains
	degree = 0
	min_remaining_val = float('inf') # min remaining value
	for var in domains:
		if not assignment.isAssigned(var):
			curr_degree = 0
			for constraint in csp.binaryConstraints:
				if constraint.affects(var):
					if not assignment.isAssigned(constraint.otherVariable(var)):
						curr_degree += 1
			curr_mrv = len(domains[var])
			# find the value with least number of options
			if curr_mrv < min_remaining_val:
				min_remaining_val = curr_mrv
				nextVar = var
				degree = curr_degree
			# tiebreaker
			if curr_mrv == min_remaining_val and curr_degree > degree:
				min_remaining_val = curr_mrv
				nextVar = var
				degree = curr_degree
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	"""Question 3
	This takes in a variable and determines the order in which the possible values should be attempted according to the 
	least constraining values heuristic, which prefers values that eliminate the fewest possibilities from other variables
	"""
	values = list(assignment.varDomains[var])
	domains = assignment.varDomains
	unsorted_list = []
	for value1 in values:  # why do you care for assigned values
		priority = 0
		for constraint in csp.binaryConstraints:
			if constraint.affects(var):
				for value2 in domains[constraint.otherVariable(var)]:
					if not constraint.isSatisfied(value2, value1):
						priority += 1
		# assign a priority to the value
		unsorted_list.append((value1, priority))
	# key argument with lamba callable function to sort the list
	unsorted_list.sort(key=lambda tuples: tuples[1])
	values = []
	for val, prior in unsorted_list:
		values.append(val)
	return values
...
def forwardChecking(assignment, csp, var, value):
	...
	domains = assignment.varDomains
	"""Question 4:
		Inferences are a way to identify impossible assignments early on 
			by looking at how a new value assignment affects other variables
		...
		if constraint.affects(var):
			for value2 in domains[constraint.otherVariable(var)]:
				if not constraint.isSatisfied(value, value2):
					inferences.add((constraint.otherVariable(var), value2))
...
def recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod):
	...
	for value in orderValuesMethod(assignment, csp, var):
		if consistent(assignment, csp, var, value):
			...
			# similar to q1 backtracking, adding inferences
			if inferences != None:
...
def revise(assignment, csp, var1, var2, constraint):
	"""
	This is a helper function that is responsible for determining inconsistent values in a variable (Section 6.2.2, Figure 6.3).
	"""
	"""Question 5"""
	"""YOUR CODE HERE"""
	inferences = set([])
	...
	for x in domains[var2]:
		...
		for y in domains[var1]:
			# if the variables are satisfied
			if constraint.isSatisfied(x, y):
				curr_remove = 0
				break
		# add variable to list to be removed if they don't satisfy constraint
		if curr_remove == 1:
			val_to_be_removed.append(x)

	for x in val_to_be_removed:
		domains[var2].remove(x)
		inferences.add((var2, x))
		# when there is no more to be removed
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	The difference is that is uses a queue to propagate these changes to other related variables
	"""
	"""Question 5"""
	"""YOUR CODE HERE"""
	inferences = set([])
	...
	while len(queue) != 0:
		curr_constraint = queue[0]
		queue.remove(curr_constraint)
		var1 = curr_constraint[0]
		var2 = curr_constraint[1]
		# use revise as helper function to determine inconsistent values
		infer = revise(assignment, csp, var1, var2, curr_constraint[2])
		...
		if len(infer) != 0:
			for constr in csp.binaryConstraints:
				if constr.affects(var2):
					queue.append((var2, constr.otherVariable(var2), constr))
			inferences = inferences.union(infer)
...
def AC3(assignment, csp):
	inferences = set([])
	"""Hint: implement revise first and use it as a helper function
	This algorithm is almost identical to MAC in that it propagates inferences in order to remove as many values as 
	possible. The major difference is that instead of starting with one variable, AC3 begins with all variables already 
	in the queue. Thus, reusing MAC is a fast way to implement this AC3. It also does not need to track the inferences 
	that are made, because if the assignment fails at any point then there is no prior state to back up to. This means 
	that there is no solution to the CSP
	"""
	"""Question 6"""
	domains = assignment.varDomains
	queue = deque()
	...
	while len(queue) != 0:
		head, tail, curr_constraint = queue.popleft()
		# use revise as helper function to determine inconsistent values
		infer = revise(assignment, csp, head, tail, curr_constraint)
		if infer == None:
			return None
		if len(infer) != 0:
			for constraint in csp.binaryConstraints:
				if constraint.affects(tail):
					queue.append((tail, constraint.otherVariable(tail), constraint))
